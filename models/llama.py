...
def get_tokenizer(model_name):
    ...
    assert model_size in ["7b", "13b", "70b"], "Invalid model size. Choose from 7b, 13b, or 70b."

    tokenizer = LlamaTokenizer.from_pretrained(
        llama2_model_string(model_size, chat),
        use_fast=False,
        token=HF_TOKEN  # Ensure token is used for gated models
    )
...
def get_model_and_tokenizer(model_name, cache_model=False):
    ...
    model = LlamaForCausalLM.from_pretrained(
        ...
        token=HF_TOKEN  # Ensure authentication for gated models
    )
    model.eval()
...
def llama_nll_fn(model_name, input_arr, target_arr, settings: SerializerSettings, transform, count_seps=True, temp=1, cache_model=True):
